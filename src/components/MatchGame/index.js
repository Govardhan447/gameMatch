...
const updateList = imagesList.map((item, index) => ({
  id: index,
...
class MatchGame extends Component {
  ...
  getRandomId = id => {
    this.setState({randamImageId: id})
  }

  renderRandamImage = () => {
    const {imageListItems} = this.state

    const randamImageId = Math.round(Math.random() * 30)
    const randomImage = imageListItems.filter(item => randamImageId === item.id)
    const {imageUrl} = randomImage[0]

    this.getRandomId(randomImage[0].id)
    return (
      <div className='random-image-container'>
        <img className='random-imgae' src={imageUrl} alt='match' />
      </div>
    )
  }
  ...
  matchingImage = id => {
    ...
    const matchingStatus = randomImageId === id
    this.setState({isMatching: matchingStatus})
    if (matchingStatus) {
  ...
  nonRandamImage = () => {
    const {randomImageId, imageListItems} = this.state
    const constantImg = imageListItems.filter(item => randomImageId === item.id)
    const {imageUrl} = constantImg[0]
    return (
      <div className='random-image-container'>
        <img className='random-imgae' src={imageUrl} alt='match' />
      </div>
  ...
  render() {
    const {tabsItems, activeTabId, imageListItems, isRandom} = this.state
      ...
      <div className='bg-container'>
        <nav className='navbar-container'>
          ...
          <div className='score-container'>
            <p className='score'>
              Score: <span className='span-num'>0</span>
            </p>
            <div className='timer-logo-container'>
              <img
                ...
                alt='timer'
              />
              <p className='span-num'>0 Sec</p>
            </div>
          </div>
        </nav>
        {isRandom ? this.renderRandamImage() : this.nonRandamImage()}
